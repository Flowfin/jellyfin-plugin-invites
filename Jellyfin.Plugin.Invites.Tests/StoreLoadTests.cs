...
public class StoreLoadTests
{
    ...
    /// <summary>
    /// A load with no account list claims the directory and compares nothing.
    /// An account list nobody could read is not an empty one: compared against
    /// nothing, every account the store claims reads as one the server lost.
    /// </summary>
    [Fact]
    public void ALoadWithNoAccountListClaimsTheDirectoryAndComparesNothing()
    {
        using var directory = new OwnedDirectory();
        new InvitationStore(directory.Path)
            .Write([ARecordClaiming(_accountTheServerLost)]);

        using var load = StoreLoad.Of(directory.Path, "kitchen-server", 4242, new TestClock(_started), null);

        Assert.True(load.HoldsTheStore);
        Assert.Null(load.Report);
    ...
    /// </summary>
    [Fact]
    public void ALoadRefusesAMissingClock()
    {
        using var directory = new OwnedDirectory();
