...
public sealed class LoadOnStart : IHostedService, IDisposable
{
    ...
    /// <param name="users">The server's own account list.</param>
    /// <param name="clock">The time source the claim is stamped from.</param>
    /// <param name="logger">Where the answer goes.</param>
    public LoadOnStart(IStoreDirectory directory, IUserManager users, IClock clock, ILogger<LoadOnStart> logger)
    {
    ...
    private Guid[] AccountsTheServerHas()
    {
        return _users.GetUsersIds().ToArray();
    }
