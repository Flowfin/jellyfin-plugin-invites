...
| Account template | 90 | What an invitation is worth to the account it creates. The field-by-field assertion in #69 is the control here; this floor is what keeps the type's own refusals exercised while that is written. |
...
| Controllers | 70 | Moderate, because the logic in a route is thin by design and the authorization on it is held by the inventory test from #83 rather than by coverage. **No controller exists yet**, so this row is a floor waiting for an area, and the gate says `empty` for it rather than reporting a pass. |
