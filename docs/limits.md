...
IT HAPPENED AGAIN, IN THE SAME SHAPE, AND THE RUNS BELOW ARE ALL RE-MADE. The ten
runs for the six entries were made at `9469fe0` and the three under the uninstall
entry at `30d37f9`, and this page has been edited since without either set being
made a second time:

    git rev-list --count 9469fe0..origin/master
    135
    git rev-list --count 30d37f9..origin/master
    88

So the declaration this section opens with had been false for a hundred and
thirty-five commits, and it was visible in the pastes themselves rather than
hidden: the totals beside them said the suite held 539 tests and then 584, at a
commit where it holds 657. Every one of the thirteen is re-run at the commit this
change lands on. Four of the `sed` addresses had moved again, and two of the ten
redden one test more than they did, which is named at each of the two.

What moved the four addresses and added the two failures is the retention sweep
from #59, in both directions at once: it moved the routines these faults are
applied to and it brought tests that ask the same routines the same questions.

    git log --oneline 9469fe0..origin/master -- Jellyfin.Plugin.Invites/Invitations/InvitationOperations.cs
    8234f5f Ask a real server whether it holds the sweep, and repair what the sweep makes false, for #59
    9b8efd2 Sweep invitation records past their retention period, for #59
    git log --oneline 9469fe0..origin/master -- Jellyfin.Plugin.Invites/Redemption/RedemptionDecision.cs
    9b8efd2 Sweep invitation records past their retention period, for #59
...
is the slip that puts a live code in the store file:
    ...
    $ sed -i '262s/templateLabel: templateLabel);/templateLabel: code);/' Jellyfin.Plugin.Invites/Invitations/InvitationOperations.cs
    ...
    Fehler!      : Fehler: 3, erfolgreich: 646, übersprungen: 8, gesamt: 657
...
inverted by dropping one character:
    ...
    Fehler!      : Fehler: 6, erfolgreich: 643, übersprungen: 8, gesamt: 657
...
offset cannot see:
    ...
    $ sed -i '235s/if (now >= record.ExpiresAt)/if (now.DateTime >= record.ExpiresAt.DateTime)/' Jellyfin.Plugin.Invites/Redemption/RedemptionDecision.cs
    $ dotnet build --configuration Release --no-restore && dotnet test --configuration Release --no-build
    ...
    Fehler!      : Fehler: 2, erfolgreich: 647, übersprungen: 8, gesamt: 657
...
assertion. The first reddens four now and the second two, and both grew for one
reason: #33's ceiling and #59's sweep ask the same two reading routines the same
question, so a routine taught to hide what the clock has passed answers all of
them wrongly at once. That is the ceiling and the sweep being held by the same
reading rather than by second ones, and it is why the two counts here move when
neither of these entries does:

    $ sed -i '279s/.*/            return Store().Read().Invitations.Where(invitation => invitation.ExpiresAt > _clock.UtcNow).ToImmutableArray();/' Jellyfin.Plugin.Invites/Invitations/InvitationOperations.cs
    $ dotnet build --configuration Release --no-restore && dotnet test --configuration Release --no-build
      ...
      RetentionSweepTests.AnExpiredRecordInsideItsRetentionPeriodStays [FAIL]
    Fehler!      : Fehler: 4, erfolgreich: 645, übersprungen: 8, gesamt: 657

    $ sed -i '293s/invitation.Id == id);/invitation.Id == id \&\& invitation.ExpiresAt > _clock.UtcNow);/' Jellyfin.Plugin.Invites/Invitations/InvitationOperations.cs
    $ dotnet build --configuration Release --no-restore && dotnet test --configuration Release --no-build
      ExpiryIsNotDeletionTests.AnInvitationPastItsExpiryIsStillFoundByItsIdentifier [FAIL]
      RetentionSweepTests.SurvivingRecordsComeThroughUnchanged [FAIL]
    Fehler!      : Fehler: 2, erfolgreich: 647, übersprungen: 8, gesamt: 657

The third reddens one test and is the one worth reading, because it does not
filter anything a caller sees. It writes the shortened list back, which is the tidy-up somebody adds to a
...
THIS PARAGRAPH SAID THREE ENTRIES AND NAMED UNINSTALL AMONG THEM, AND THEN SAID
NO FAULT WAS RUN AGAINST ANY OF THEM. Three faults are run against the uninstall
entry's guard below, in this section, and the paragraph that closes them says in
as many words that they do not move that entry. So the count and the claim were
each true of something and wrong together, which is this page's own subject met
on the page. Two paragraphs further down already say `these two`, and the
sentence below already says `the first of the two`; only this one said three.

The other two entries are the username disclosure and the deleted library. No
fault was run against either of them here, and for both there is nothing to run
one against. Nothing in the plugin judges a username, and nothing in it resolves
a library identifier or notices one that has gone:
