...
accumulated while the second table stood still. Both directions are written here
rather than remembered, and both are executed now, on two routes rather than one.
...
And the target gate's directory against the first one, which is decided weekly and on
demand rather than on a route a merge takes. That listing lives on another
repository, so reading it is a network call, and a failed call and a wrong ledger
reach a comparison as the same empty listing: on a pull request that would make a
timeout indistinguishable from a defect. So it is asked on a schedule, the fetch is a
step of its own so a failed call fails as a failed call, and the price is stated
rather than hidden - a workflow added on that gate on a Tuesday is unanswered until
the next run. Each run of either route prints that it did not evaluate the other, so
neither green mark is read as both:
...
The second command used to be the whole of what stood behind that direction, which is
how `perf-baseline.yml` sat on the target gate with no row for months: a listing this
tree does not hold is a listing nobody runs into by accident. Both patterns now carry the
...
| `manifest-freshness.yml` | adopted, landed | #119 landed it as `manifest-freshness.yaml`, and the row it replaces said there is no published manifest to keep fresh until a release process exists. The release process exists, in `docs/RELEASING.md`, and the manifest the decision on #155 distributes from is a document this repository does not own and cannot see fail. What did not change is that nothing is published yet, so the comparison leg is green about two empty sets today; the leg that keeps its meaning meanwhile is the selftest, which hands the reader every answer it refuses on every run. |
...
| `publish-failure-alert.yml` | adopted, landed | #376 landed it as `failure-alert.yaml`. The row it replaces said it alerts on a publishing failure and cannot land before publishing does, and that reading came from the file's name rather than from the file: on the target gate it is a default-branch watchdog that asks the run list which workflows concluded non-success on the default branch, names no workflow, and files or closes one issue, and nothing in it is about publishing. What it exists against had already happened here, in the weekly mutation gate that failed on every run it made with no reader. Reports `A red run on the default branch is reported`, on a schedule and on demand and on no pull request, so the context never appears on a pull request head, which is what #24 has to read this row for. |
...
| Workflow | Answer | Reason |
| --- | --- | --- |
| `abi-floor.yaml` | kept | This repository's own, landed by #15 at `b80b85b2`. It held the floor behind a switch while the shipping build compiled against a newer release of the line, and that package did not load at the floor, which #155 measured on a server: every build compiles against the floor since, and this workflow builds it that way with the committed lock file and then installs the packaged plugin on the published image of the floor and reads the server's plugin list back. Reports `Build against the declared ABI floor` and `Load on the declared ABI floor server`. It is a file of its own rather than a leg inside the build, which is what the `dotnet.yml` row above says #15 does to that workflow on the target gate. |
...
| `e2e-no-web-client.yaml` | kept | This repository's own, landed under #74. That issue chose an embedded page over a redirect into the web client so the flow survives a server with no client, and everything holding that read this repository: the suite asserts the bytes the route returns, and `e2e-authorization.yaml` asks a real server that carries the client, which is a different question. This job starts the same pinned image with `--nowebclient`, confirms the client is gone before it asks for anything else, and compares the served page against the tracked file byte for byte. Reports `The setup page is served whole with no web client installed`. It is the fourth of the same family and same trigger as the three above, so its context appears on a pull request head. Nothing on the target gate's list answers to it. What it does not answer is whether a browser renders those bytes, which stays with the recorded manual check. |
| `e2e-plugin-disabled.yaml` | kept | This repository's own, landed under #47. That issue's first clause asks that disabling the plugin make every redemption route unreachable, and no reading of this tree answers it: what a host does to a route a disabled plugin registered at startup lives in the server's own assemblies, which this plugin does not reference. So this job installs the packaged plugin on the same pinned image the four rows around it use, disables it through the server's own administrator route, and reads the public redemption address twice - once with the server still running and once after a restart - because an operator meets both moments and a job measuring one would be right about half of them. Reports `Disabling the plugin is put to a server`. It is the fifth of the same family and asks a fifth question: that pair asks what an unauthenticated request gets and what the server holds as a plugin, the scheduled-task row asks what it holds as a task, the no-web-client row asks what it serves without a client, and this one asks what it still routes to once an operator has switched the plugin off. Nothing on the target gate's list answers to it. Same trigger as the four rows above, so its context appears on a pull request head too. What it refuses is the four readings a run was seen to produce: the route answering 200 with the server still running, 404 after a restart, this plugin absent from the server's plugin list afterwards, and the enable route answering 404 on it. Each of the four is a sentence `docs/operator-guide.md` now tells an operator, so a server line where one of them moves reds this job instead of leaving that page saying something the server stopped doing. None of the four is asserted as desirable, and the first is the one the guide's warning is about. |
| `e2e-scheduled-task.yaml` | kept | This repository's own, landed under #59. A scheduled task is the one thing this plugin builds that the suite cannot invoke the way the server does: `MediaBrowser.Model` carries the interface and nothing else, and how a server discovers an implementation lives in assemblies this project does not reference. So this job installs the packaged plugin on the same published image the two rows above use, completes the wizard for a credential, and reads the server's own scheduled task list. It is the third of the same family rather than a second copy of either: that pair asks what an unauthenticated request gets and what the server holds as a plugin, this one asks what the server holds as a task. Reports `The server's scheduler holds the retention sweep`. Nothing on the target gate's list answers to it. Same trigger as the two rows above, so its context appears on a pull request head too. |
| `failure-alert.yaml` | kept | This repository's answer to the target gate's `publish-failure-alert.yml`, landed under #376. A run that is not a required pull-request check is mailed to nobody the way a red pull request is, so the weekly mutation gate failed on every run it made and its only reader was somebody who opened the actions page. This reads the latest completed run of every workflow on the default branch, hands the listing to `.github/lint/failure-alert.sh`, and for each run that concluded non-success files or refreshes one issue carrying the failed step and the end of its log, then closes that issue when nothing is red. No workflow is named in it, so one landed tomorrow is watched from its first run, and this file is in the set it watches. Reports `A red run on the default branch is reported`, every six hours and on demand and on no pull request, because the run list is a network call and a failed call and a quiet default branch arrive as the same empty listing; the fetch is a step of its own so a failed call fails as a failed call. The price is stated on the file: a run that goes red is reported at the next tick rather than within a minute. It must not go in the required set, for the reason its neighbours give. |
...
| `issue-pointer.yaml` | kept | This repository's own, landed under #427. Some tables here carry a column naming the issue that will do a thing, and a reader follows it to find out whether the thing has arrived; an issue that has closed reads as arrived. The class was repaired by hand five times before anything read it. It refuses a cell holding a closed issue reference under a column its register names, a cell naming a number the listing does not carry, a register line naming a file or a heading that is not there, and a run that read nothing. The register is why it is not every table: the mitigation column of `docs/threat-model.md` has the same bytes and the opposite direction, where a closed number means the defence landed. Reports `Every forward-pointing column names open work`, daily and on demand and on no pull request, for the reason its neighbour `tracker-claim.yaml` gives: reading the tracker is a network call, and a pointer goes stale when somebody closes an issue rather than when somebody edits a file. What it does not answer is a cell pointing at an issue that is open and is the wrong one, which is a judgement about meaning. Nothing on the target gate's list answers to it. |
| `manifest.yaml` | kept | This repository's own, landed under #119. The catalogue entry a server installs from is generated from `build.yaml` and from the archive that was built, and its checksum is the field that decides what gets installed, so a generator that had stopped refusing a malformed field would be found by a broken release rather than by a red mark. The release route runs a few times a year at most; this runs the generator's selftest and generates an entry from the tracked metadata on every push, so the reader is exercised between releases. Reports `The manifest entry generates from the metadata in the tree`. Nothing on the target gate's list answers to it. What the generator deliberately does not do, including the one refusal no fixture reaches, is written at the top of `.github/lint/manifest.sh`. |
| `manifest-freshness.yaml` | kept | This repository's answer to the target gate's `manifest-freshness.yml`, landed under #119. A server installs whatever the manifest at the address in `docs/distribution.md` offers, and that document is not in this tree: a release that was published and never reached it is a release nobody can install, and the publishing run that shipped it is green, because the step that failed is the step that would have had to report it. This fetches that document daily and reads it against the releases this repository has, refusing a manifest behind the newest release, ahead of every release, missing the entry, missing the server line, or offering something never published. Reports `The manifest a server fetches offers what was released` and, in a second job, `The freshness check refuses what it says it refuses`. That second job is not a formality: this repository has published nothing and the hub carries no entry for it, so the comparison is green about two empty sets and would stay green whatever the reader did, which is why the reader is handed every answer it refuses from fixtures on every run. Nothing on the target gate's list answers to it beyond the row above. What it deliberately does not do, including that a red run raises nothing anybody sees, is written at the top of `.github/lint/manifest-freshness.sh` and in `docs/distribution.md`. |
...
| `tracker-claim.yaml` | kept | This repository's own, landed under #100. A status line saying an issue is open or closed is a claim about a state something else holds, and two rows of `docs/tests-not-written.md` went stale in exactly that sentence while the page went on reading as covered. It refuses a present-tense claim in tracked markdown that the tracker disagrees with, a claim naming a number the listing does not carry, and a run that found no claim at all. Reports `Every claim about an issue's state still holds`, daily and on demand and on no pull request, because reading the tracker is a network call and a failed call reaches the comparison as the same missing entries a wrong page does; a claim here also goes stale when somebody closes an issue rather than when somebody edits a file, so a merge is not the moment the answer changes. Nothing on the target gate's list answers to it. |
| `workflow-parity.yaml` | kept | This repository's own, landed under #12. This page carried two hand checks somebody had to remember to run, and four times nobody did: a workflow landed, the table went on describing the directory as it had been, and the change was green. This decides the direction a command over this tree can decide, on every push. It refuses a workflow file with no row, a row that owes a file and names none, and a row saying a workflow was removed while the file is here. Reports `The ledger accounts for every workflow`. Nothing on the target gate's list answers to it, and the direction it does NOT evaluate is the first table above, which every run of it prints. `workflow-parity-target.yaml` is what evaluates that one, and it is a second file rather than a second job here because it needs a network call and this one must not have one. |
| `workflow-parity-target.yaml` | kept | This repository's own, landed under #12. It holds the first table above to the target gate's own workflow directory, which is the direction a command over this tree cannot decide and the one `perf-baseline.yml` drifted in unnoticed. Reports `The ledger accounts for the target gate`, weekly and on demand and on no pull request, because reading another repository is a network call and a failed call reaches a comparison as the same empty listing a wrong ledger does; on a merge route that is a red gate somebody learns to re-run rather than to read. It fetches in a step of its own so a failed call fails as a failed call. Nothing on the target gate's list answers to it. |
